...
- 🚗 모든 음악을 한 폴더에 `Artist - Title.mp3` 형식으로 저장.
...
4. 모호한 결과를 검토한 뒤 **음악 다운로드**를 누르세요. 전체 검색과 진행 중인 수정이 끝나야 다운로드할 수 있습니다.
...
다운로드 전에 선택한 폴더 바로 아래의 MP3를 검사합니다. YouTube ID/주소 태그가 있으면 DB가 없어지거나 파일명·경로가 달라져도 재사용합니다. 이전 버전의 태그도 인식하며, 메타데이터 변경 시 음원을 재다운로드·재인코딩하지 않습니다. 태그 없는 파일이나 손상 파일은 파일명으로 추측하지 않습니다.
